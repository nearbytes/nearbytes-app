...
7. The real remaining blocker is lower in the stack: most of the shared MEGA runtime in `src/integrations/mega.ts` is still Node-bound.
...
3. Provider-owned remote actions still need a final architecture choice per operation:
...
4. Functional parity with desktop has not yet been reached, so the iPhone path is not ready to be described as complete.
...
2. Reconnect the phone provider path to the shared MEGA session/contact-invite/reconciliation logic once those shared pieces are phone-safe.
...
1. Prove that the phone can import and exercise the shared MEGA protocol/session helpers without pulling in desktop-only runtime code.
2. Identify the smallest MEGA session/auth/contact-invite slice that can be shared on phone next.
3. Only after that, decide which remaining MEGA runtime seams need native ownership.
