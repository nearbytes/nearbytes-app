...
- [ ] Make the shipped app import and use the Svelte spec components from `docs/specs/ui` as the canonical design source. Commit:
	Runtime shell imports now resolve from `docs/specs/ui/components`, shared tokens/styles now resolve from `docs/specs/ui/tokens.ts` and `docs/specs/ui/global.css`, and `?design` mounts the spec app directly. Remaining app-owned design surfaces still need the same migration.
