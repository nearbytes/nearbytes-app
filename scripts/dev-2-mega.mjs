...
const localLogPath = path.join(logsDir, 'dev-2-mega-local.log');
const remoteLogPath = path.join(logsDir, 'dev-2-mega-remote.log');
...
try {
  console.error('[dev-2-mega] preparing dual local dev environment');
  ...
  console.error('[dev-2-mega] building project');
  ensureRepoBuild();
  console.error('[dev-2-mega] clearing dedicated ports', { localPort, remotePort });
  await clearPorts([localPort, remotePort]);

  const localDesktopStartedAt = Date.now();
  console.error('[dev-2-mega] starting local desktop target', { apiPort: localPort, uiPort: localUiPort, home: localHome });
  ...
  console.error('[dev-2-mega] starting remote web target', { apiPort: remotePort, uiPort: remoteUiPort, home: remoteHome });
  ...
  console.error('[dev-2-mega] waiting for both instances to become healthy');
  ...
  console.error('[dev-2-mega] desktop and remote targets are healthy');

  const localDesktopHeaders = {
    'x-nearbytes-runtime-token': localDesktopSession.token,
  };

  console.error('[dev-2-mega] connecting local MEGA account', { email: localEmail, baseUrl: localBaseUrl });
  const localAccount = await ensureExpectedMegaAccount(localBaseUrl, localEmail, localDesktopHeaders);
  console.error('[dev-2-mega] connecting remote MEGA account', { email: remoteEmail, baseUrl: remoteBaseUrl });
  ...
  if (shouldWipe) {
    ...
    console.error('[dev-2-mega] wiping both MEGA test accounts');
    ...
    console.error('[dev-2-mega] wipe completed');
  } else {
    console.error('[dev-2-mega] wipe skipped by user; continuing with connected instances');
  }

  console.error('[dev-2-mega] MEGA accounts connected and wiped', {
  ...
  console.error('[dev-2-mega] ready for manual end-to-end testing');
  console.error(`[dev-2-mega] local desktop UI: ${localUiUrl}`);
  console.error(`[dev-2-mega] remote UI: ${remoteUiUrl}`);
  console.error('[dev-2-mega] local target is the desktop app; remote target opens in the browser');
  console.error('[dev-2-mega] press Ctrl-C to stop both instances');

  await Promise.all(children.map((child) => waitForExit(child)));
  await shutdown(0);
} catch (error) {
  console.error('[dev-2-mega] FAILED', error instanceof Error ? error.stack ?? error.message : String(error));
  await shutdown(1);
}
