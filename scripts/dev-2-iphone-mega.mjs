...
const desktopHome = trimOrDefault(process.env.NEARBYTES_DEV2_IPHONE_MEGA_DESKTOP_HOME, '/tmp/nearbytes-dev-2-iphone-mega-desktop-home');
const phoneHome = trimOrDefault(process.env.NEARBYTES_DEV2_IPHONE_MEGA_PHONE_HOME, '/tmp/nearbytes-dev-2-iphone-mega-phone-home');
const logsDir = path.join(os.homedir(), '.nearbytes', 'logs');
const desktopLogPath = path.join(logsDir, 'dev-2-iphone-mega-desktop.log');
const phoneBackendLogPath = path.join(logsDir, 'dev-2-iphone-mega-phone-backend.log');
const phoneUiLogPath = path.join(logsDir, 'dev-2-iphone-mega-phone-ui.log');
...
try {
  console.error('[dev-2-iphone-mega] preparing desktop + iPhone MEGA environment');
  console.error('[dev-2-iphone-mega] loaded MEGA test accounts from .env.e2e');
  console.error(`[dev-2-iphone-mega] phone account: ${phoneEmail}`);
  console.error(`[dev-2-iphone-mega] desktop account: ${desktopEmail}`);
  console.error(`[dev-2-iphone-mega] wipe mode: ${describeWipeMode(wipeMode)}`);
  ...
  ensureRepoBuild();
  console.error('[dev-2-iphone-mega] clearing dedicated ports', { desktopUiPort, phoneApiPort, phoneUiPort });
  await clearPorts([desktopUiPort, phoneApiPort, phoneUiPort]);

  console.error('[dev-2-iphone-mega] preseeding phone MEGA account', { email: phoneEmail });
  ...
  console.error('[dev-2-iphone-mega] preseeding desktop MEGA account', { email: desktopEmail });
  await preseedMegaAccount({
    email: desktopEmail,
    password: megaPassword,
  ...
  console.error('[dev-2-iphone-mega] starting desktop runtime');
  ...
  console.error('[dev-2-iphone-mega] starting phone backend');
  ...
  console.error('[dev-2-iphone-mega] waiting for desktop runtime and phone backend');
  ...
  console.error('[dev-2-iphone-mega] starting phone UI dev server');
  ...
  console.error('[dev-2-iphone-mega] launching iPhone simulator app');
  ...
  if (shouldWipe) {
    ...
    console.error('[dev-2-iphone-mega] wiping both MEGA test accounts');
  ...
  console.error('[dev-2-iphone-mega] ready for manual MEGA testing');
  console.error(`[dev-2-iphone-mega] desktop UI: http://127.0.0.1:${desktopUiPort}`);
  console.error(`[dev-2-iphone-mega] desktop API: ${desktopApiUrl}`);
  console.error(`[dev-2-iphone-mega] phone UI: ${phoneUiUrl}`);
  console.error(`[dev-2-iphone-mega] phone API: ${phoneApiUrl}`);
  console.error(`[dev-2-iphone-mega] desktop dev api descriptor: ${path.join(desktopPaths.nearbytesDir, 'dev-api.json')}`);
  console.error(`[dev-2-iphone-mega] phone dev api descriptor: ${path.join(phonePaths.nearbytesDir, 'dev-api.json')}`);
  console.error('[dev-2-iphone-mega] both participants run with MEGA enabled and LAN disabled');
  console.error('[dev-2-iphone-mega] connected accounts', {
    phoneAccountId: phoneAccount.id,
    desktopAccountId: desktopAccount.id,
  });
  console.error('[dev-2-iphone-mega] press Ctrl-C to stop all processes');

  await Promise.all(children.map((child) => waitForExit(child)));
  await shutdown(0);
} catch (error) {
  console.error('[dev-2-iphone-mega] FAILED', error instanceof Error ? error.stack ?? error.message : String(error));
  await shutdown(1);
}
