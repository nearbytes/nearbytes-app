...
describe('MegaTransportAdapter', () => {
  ...
  it('resolves owner share crypto from cached root-handle share keys', async () => {
    ...
    it('reloads persisted owner share keys when the in-memory cache misses', async () => {
      const secretStore = createMemorySecretStore();
      const email = 'owner-persisted@example.com';
      const userHandle = 'ownrps001';
      const shareHandle = 'cIVQ2bjB';
      const shareKey = Buffer.from('00112233445566778899aabbccddeeff', 'hex');
      const masterKey = Buffer.from('11223344556677889900aabbccddeeff', 'hex');

      await secretStore.set('provider-account:mega:acct-mega-owner-persisted', {
        email,
        password: 'secret',
        sid: 'sid-owner-persisted',
        masterKey: encodeMegaBase64Url(masterKey),
        userHandle,
        accountVersion: 2,
        shareKeys: {
      ...
      const session = await adapterInternal.getAccountSession(account);
      const root = { root: { handle: shareHandle, shareHandle } };
      adapterInternal.accountShareKeyCache.delete(userHandle);

      const shareCrypto = await adapterInternal.resolveOwnerShareCryptoContext(session, root);

      expect(shareCrypto?.shareHandle).toBe(shareHandle);
      expect(shareCrypto?.shareKey).toEqual(shareKey);
      expect(adapterInternal.accountShareKeyCache.get(userHandle)?.get(shareHandle)).toEqual(shareKey);
    });
  ...
  it('prioritizes runtime-source channel uploads and skips vanished files without aborting owner sync', async () => {
    const secretStore = createMemorySecretStore();
    const email = 'owner@example.com';
    const userHandle = 'owner001';
    const driveHandle = 'root0001';
    ...
    await secretStore.set('provider-account:mega:acct-mega-owner-runtime-priority', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(masterKey),
      ...
      if (url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
          ...
          case 'p': {
            const nodes = Array.isArray(payload.n) ? payload.n : [];
            const node = nodes[0] && typeof nodes[0] === 'object' ? (nodes[0] as Record<string, unknown>) : null;
            const handle = typeof node?.h === 'string' ? node.h : `upload-${uploadedPaths.length}`;
            const attributes = typeof node?.a === 'string' ? node.a : '';
            const parentHandle = typeof payload.t === 'string' ? payload.t : '';
            const cr = Array.isArray(payload.cr) ? payload.cr : [];
            const records = Array.isArray(cr[2]) ? cr[2] : [];
            ...
            uploadedFiles.set(relativePath, {
              handle,
              size: relativePath === 'channels/runtime.bin' ? 3 : 4,
              attributes,
              nodeKey,
            });
            return new Response(JSON.stringify([{}]), {
    ...
    const account: ProviderAccount = {
      id: 'acct-mega-owner-runtime-priority',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-owner-runtime-priority',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'owner',
      localPath: '/logical/nearbytes',
      sourceId: 'src-mega-owner-runtime-priority',
      syncMode: 'mirror',
      remoteDescriptor: {
        remotePath: '/nearbytes',
        shareName: 'nearbytes',
  ...
  it('uploads referenced runtime-source blocks before publishing a channel event during owner sync', async () => {
    ...
    const uploadedFiles = new Map<
      string,
      { handle: string; parentHandle: string; size: number; attributes: string; nodeKey: Buffer }
    >();
      ...
      if (url.startsWith('https://g.api.mega.co.nz/cs?')) {
        ...
        switch (payload.a) {
          ...
          case 'p': {
            ...
            const nodes = Array.isArray(payload.n) ? payload.n : [];
            const node = nodes[0] && typeof nodes[0] === 'object' ? (nodes[0] as Record<string, unknown>) : null;
            const handle = typeof node?.h === 'string' ? node.h : `uploadtok-${uploadedPaths.length}`;
            const attributes = typeof node?.a === 'string' ? node.a : '';
            const cr = Array.isArray(payload.cr) ? payload.cr : [];
            const records = Array.isArray(cr[2]) ? cr[2] : [];
            const encodedShareKeyRecord = typeof records[2] === 'string' ? records[2] : '';
            ...
            uploadedFiles.set(relativePath, {
              handle,
              parentHandle,
              size: relativePath === 'channels/runtime.bin' ? eventBytes.length : blockBytes.length,
              attributes,
              nodeKey,
            });
            return new Response(JSON.stringify([{}]), {
  ...
  it('waits for referenced runtime-source blocks to become visible before publishing a channel event', async () => {
    vi.useFakeTimers();
    const secretStore = createMemorySecretStore();
    const email = 'owner@example.com';
    const userHandle = 'owner001';
    ...
    const channelsNodeKey = Buffer.from('2233445566778899aabbccddeeff0011', 'hex');
    const uploadedPaths: string[] = [];
    const visibleBlockPaths = new Set<string>();
    const visibleChannelPaths = new Set<string>();
    const uploadedFiles = new Map<
      string,
      { handle: string; parentHandle: string; size: number; attributes: string; encryptedNodeKey: string }
    >();
    let blockVisibilityPolls = 0;

    await secretStore.set('provider-account:mega:acct-mega-owner-runtime-block-visibility', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(masterKey),
    ...
    const blockHash = '38cc41f2bdd578c2fe3c50bbefaedc86dcf8fbb4a9839458a28f67f6d350bb1a';
    const cryptoOps = createCryptoOperations();
    const volumeKeyPair = await cryptoOps.deriveKeys(createSecret('mega-runtime-owner-block-visibility'));
    const storedEvent = await createSignedEvent(
      cryptoOps,
      volumeKeyPair,
      {
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith('https://g.api.mega.co.nz/cs?')) {
        ...
        switch (payload.a) {
          ...
          case 'f': {
            const requestedRoot = typeof payload.n === 'string' ? payload.n : undefined;
            if (requestedRoot === blocksHandle) {
              blockVisibilityPolls += 1;
              if (blockVisibilityPolls >= 2) {
                visibleBlockPaths.add(`blocks/${blockHash}.bin`);
              }
            }
            const nodes: Array<Record<string, unknown>> = [
              {
            ...
            if (visibleBlockPaths.has(`blocks/${blockHash}.bin`)) {
              const uploadedBlock = uploadedFiles.get(`blocks/${blockHash}.bin`);
              if (uploadedBlock) {
                nodes.push({
                  h: uploadedBlock.handle,
                  p: uploadedBlock.parentHandle,
                  t: 0,
                  s: uploadedBlock.size,
                  a: uploadedBlock.attributes,
                  k: uploadedBlock.encryptedNodeKey,
                });
              }
            }
            if (visibleChannelPaths.has('channels/runtime.bin')) {
              const uploadedChannel = uploadedFiles.get('channels/runtime.bin');
              if (uploadedChannel) {
                nodes.push({
                  h: uploadedChannel.handle,
                  p: uploadedChannel.parentHandle,
                  t: 0,
                  s: uploadedChannel.size,
                  a: uploadedChannel.attributes,
                  k: uploadedChannel.encryptedNodeKey,
                });
              }
            }
            const partialNodes = requestedRoot
          ...
          case 'p': {
            ...
            const handle = typeof node?.h === 'string' ? node.h : `uploadtok-${uploadedPaths.length}`;
            const attributes = typeof node?.a === 'string' ? node.a : '';
            const encryptedNodeKey = typeof node?.k === 'string' ? node.k : '';
            const relativePath = parentHandle === channelsHandle ? 'channels/runtime.bin' : `blocks/${blockHash}.bin`;
            if (parentHandle === channelsHandle && !visibleBlockPaths.has(`blocks/${blockHash}.bin`)) {
              throw new Error('Channel event published before referenced block became visible.');
            }
            if (parentHandle === channelsHandle) {
              visibleChannelPaths.add('channels/runtime.bin');
              uploadedPaths.push('channels/runtime.bin');
            } else {
              uploadedPaths.push('blocks/runtime.bin');
            }
            uploadedFiles.set(relativePath, {
              handle,
              parentHandle,
    ...
    const ownerMirrorSource = {
      listMirrorFiles: vi.fn(async () => [`blocks/${blockHash}.bin`, 'channels/runtime.bin']),
      readMirrorFile: vi.fn(async (_share: ManagedShare, relativePath: string) => {
        if (relativePath === 'channels/runtime.bin') {
          return eventBytes;
        }
        if (relativePath === `blocks/${blockHash}.bin`) {
          return blockBytes;
        }
        throw new Error(`Unexpected runtime-source read: ${relativePath}`);
      }),
    ...
    const account: ProviderAccount = {
      id: 'acct-mega-owner-runtime-block-visibility',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-owner-runtime-block-visibility',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'owner',
      localPath: '/logical/nearbytes',
      sourceId: 'src-mega-owner-runtime-block-visibility',
      syncMode: 'mirror',
      remoteDescriptor: {
        remotePath: '/nearbytes',
        shareName: 'nearbytes',
    ...
    const syncPromise = (adapter as any).syncOwnerShare(share, account);
    await vi.runAllTimersAsync();
    await expect(syncPromise).resolves.toBeUndefined();

    expect(blockVisibilityPolls).toBeGreaterThanOrEqual(2);
  ...
  it('retries a runtime-source block read that appears shortly after the channel event', async () => {
    ...
    await secretStore.set('provider-account:mega:acct-mega-owner-runtime-block-retry', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(masterKey),
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
              ...
              JSON.stringify([
                {
                  f: [
                    ...
                    ...(uploadedBlockVisible && uploadedBlockNode
                      ? [
                          {
                            h: uploadedBlockNode.handle,
                            p: uploadedBlockNode.parentHandle,
                            t: 0,
                            s: uploadedBlockNode.size,
                            a: uploadedBlockNode.attributes,
                            k: uploadedBlockNode.encryptedNodeKey,
                          },
                        ]
                      : []),
                  ],
          ...
          case 'p': {
            const parentHandle = typeof payload.t === 'string' ? payload.t : '';
            if (parentHandle === blocksHandle) {
              uploadedBlockVisible = true;
              const nodes = Array.isArray(payload.n) ? payload.n : [];
              const node = nodes[0] && typeof nodes[0] === 'object' ? (nodes[0] as Record<string, unknown>) : null;
              uploadedBlockNode = {
                handle: typeof node?.h === 'string' ? node.h : 'uploadedblk',
                parentHandle,
                size: blockBytes.length,
                attributes: typeof node?.a === 'string' ? node.a : '',
                encryptedNodeKey: typeof node?.k === 'string' ? node.k : '',
              };
            }
            uploadedPaths.push(parentHandle === channelsHandle ? 'channels/runtime.bin' : 'blocks/runtime.bin');
            return new Response(JSON.stringify([{}]), {
    ...
    const blockHash = '2ad4ec97b89f8c58b676b16f982e02a6accbe4fc01d0e24286dee41a3bd0286a';
    const cryptoOps = createCryptoOperations();
    const volumeKeyPair = await cryptoOps.deriveKeys(createSecret('mega-runtime-block-retry'));
    const storedEvent = await createSignedEvent(
      cryptoOps,
      volumeKeyPair,
      {
    ...
    const eventBytes = new TextEncoder().encode(JSON.stringify(serializeEvent(storedEvent)));
    const blockBytes = new Uint8Array([4, 5, 6, 7]);
    let blockReadAttempts = 0;
    let uploadedBlockVisible = false;
    let uploadedBlockNode:
      | { handle: string; parentHandle: string; size: number; attributes: string; encryptedNodeKey: string }
      | undefined;
    const ownerMirrorSource = {
      listMirrorFiles: vi.fn(async () => []),
      readMirrorFile: vi.fn(async (_share: ManagedShare, relativePath: string) => {
        ...
        if (relativePath === `blocks/${blockHash}.bin`) {
          blockReadAttempts += 1;
          if (blockReadAttempts <= 4) {
            throw new Error(`File not found: blocks/${blockHash}.bin`);
          }
          return blockBytes;
        }
        throw new Error(`Unexpected runtime-source read: ${relativePath}`);
    ...
    const account: ProviderAccount = {
      id: 'acct-mega-owner-runtime-block-retry',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-owner-runtime-block-retry',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'owner',
      localPath: '/logical/nearbytes',
      sourceId: 'src-mega-owner-runtime-block-retry',
      syncMode: 'mirror',
      remoteDescriptor: {
        remotePath: '/nearbytes',
        shareName: 'nearbytes',
    ...
    const publish = adapter.handleManagedShareLocalWrite(share, account, 'channels/runtime.bin');
    await vi.runAllTimersAsync();
  ...
  it('re-uploads runtime-source channel events when the remote file has the same byte length', async () => {
    ...
    const blocksNodeKey = Buffer.from('11223344556677889900aabbccddeeff', 'hex');
    const channelsNodeKey = Buffer.from('2233445566778899aabbccddeeff0011', 'hex');
    const previousBytes = Buffer.from('prior-channel-event!', 'utf8');
    const nextBytes = Buffer.from('next!-channel-event!', 'utf8');
    expect(previousBytes.length).toBe(nextBytes.length);
    const uploadedFiles = new Map<string, { handle: string; size: number; attributes: string; nodeKey: Buffer }>([
      [
        'channels/runtime.bin',
        {
          handle: 'existingchan1',
          size: previousBytes.length,
          attributes: encryptAttributes('runtime.bin', Buffer.from('33445566778899aabbccddeeff001122', 'hex')),
          nodeKey: Buffer.from('445566778899aabbccddeeff00112233', 'hex'),
        },
      ],
    ]);
    let uploadReservationCount = 0;
    ...
    await secretStore.set('provider-account:mega:acct-mega-owner-runtime-channels', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(masterKey),
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
          ...
          case 'd':
            uploadedFiles.delete('channels/runtime.bin');
          ...
          case 'p': {
            ...
            const nodes = Array.isArray(payload.n) ? payload.n : [];
            const node = nodes[0] && typeof nodes[0] === 'object' ? (nodes[0] as Record<string, unknown>) : null;
            const handle = typeof node?.h === 'string' ? node.h : 'uploadtok02';
            const attributes = typeof node?.a === 'string' ? node.a : '';
            const parentHandle = typeof payload.t === 'string' ? payload.t : '';
            const cr = Array.isArray(payload.cr) ? payload.cr : [];
            const records = Array.isArray(cr[2]) ? cr[2] : [];
      ...
      if (url.startsWith('https://upload.test/file/0?d=')) {
        return new Response(Buffer.from('uploadtok02'), { status: 200 });
      }
      throw new Error(`Unexpected request URL: ${url}`);
    }) as typeof fetch;

    const ownerMirrorSource = {
      listMirrorFiles: vi.fn(async () => ['channels/runtime.bin']),
    ...
    const account: ProviderAccount = {
      id: 'acct-mega-owner-runtime-channels',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-owner-runtime-channels',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'owner',
      localPath: '/logical/nearbytes',
      sourceId: 'src-mega-owner-runtime-channels',
      syncMode: 'mirror',
      remoteDescriptor: {
        remotePath: '/nearbytes',
        shareName: 'nearbytes',
    ...
    expect(ownerMirrorSource.listMirrorFiles).toHaveBeenCalledWith(share);
    expect(ownerMirrorSource.readMirrorFile).toHaveBeenCalledWith(share, 'channels/runtime.bin');
    expect(uploadReservationCount).toBe(1);
    expect(uploadCommitCount).toBe(1);
  ...
  it('tries recipient immediate-apply before skipping an incremental batch as unrelated', async () => {
    const runtime = createIntegrationRuntime({
      secretStore: createMemorySecretStore(),
      logger: {
        log() {},
    ...
    const account: ProviderAccount = {
      id: 'acct-mega-recipient-incremental-apply',
      provider: 'mega',
      label: 'MEGA',
      state: 'connected',
      createdAt: Date.now(),
    ...
    const recipientShare: ManagedShare = {
      id: 'share-mega-recipient-incremental-apply',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'recipient',
      localPath: '/tmp/mega-recipient-incremental-apply',
      sourceId: 'src-mega-recipient-incremental-apply',
      syncMode: 'mirror',
      remoteDescriptor: { remotePath: 'owner@example.com:nearbytes', shareHandle: 'root0005', rootHandle: 'root0005' },
      capabilities: ['mirror', 'read', 'accept'],
      invitationEmails: [],
      createdAt: Date.now(),
      updatedAt: Date.now(),
    ...
    const session = {
      sid: 'sid',
      masterKey: Buffer.alloc(16, 1),
      userHandle: 'user0001',
      email: 'reader@example.com',
    };

    vi.spyOn(adapter as unknown as {
      getAccountSession: (account: ProviderAccount, signal?: AbortSignal) => Promise<unknown>;
    }, 'getAccountSession').mockResolvedValue(session);
    vi.spyOn(adapter as unknown as {
      resolveIncomingShareDescriptor: (
        account: ProviderAccount,
        descriptor: Record<string, unknown>,
        options?: { forceRefresh?: boolean }
      ) => Promise<Record<string, unknown>>;
    }, 'resolveIncomingShareDescriptor').mockResolvedValue(recipientShare.remoteDescriptor);
    vi.spyOn(adapter as unknown as {
      loadManifest: (shareId: string) => Promise<unknown>;
    }, 'loadManifest').mockResolvedValue({
      rootHandle: 'root0005',
      lastScsn: 'cursor-0',
      entries: {},
      knownHandles: [],
      unsupportedTopLevelNames: [],
    });
    vi.spyOn(adapter as unknown as {
      fetchActionPackets: (session: unknown, scsn: string, signal?: AbortSignal) => Promise<unknown>;
    }, 'fetchActionPackets').mockResolvedValue({
      scsn: 'cursor-1',
      packets: [
        {
          a: 'u',
    ...
    vi.spyOn(adapter as unknown as {
      rememberActionPacketShareKeys: (session: unknown, packets: Record<string, unknown>[]) => Promise<number>;
    }, 'rememberActionPacketShareKeys').mockResolvedValue(0);
    vi.spyOn(adapter as unknown as {
      updateManifestCursor: (shareId: string, scsn: string) => Promise<void>;
    }, 'updateManifestCursor').mockResolvedValue();
    const tryApplyRecipientActionPackets = vi.spyOn(adapter as unknown as {
      tryApplyRecipientActionPackets: (
        share: ManagedShare,
        account: ProviderAccount,
        rootHandle: string,
        manifest: unknown,
        actionBatch: unknown,
        baseProbeContext?: unknown,
        activeSession?: unknown,
      ) => Promise<boolean>;
    }, 'tryApplyRecipientActionPackets').mockResolvedValue(true);
    const fetchPartialTreeWithSnapshot = vi.spyOn(adapter as unknown as {
      fetchPartialTreeWithSnapshot: (session: unknown, rootHandle: string, signal?: AbortSignal) => Promise<unknown>;
    }, 'fetchPartialTreeWithSnapshot');

    await expect((adapter as unknown as {
      syncShare: (share: ManagedShare, account: ProviderAccount, signal?: AbortSignal) => Promise<void>;
    }).syncShare(recipientShare, account)).resolves.toBeUndefined();

    expect(tryApplyRecipientActionPackets).toHaveBeenCalledTimes(1);
    expect(fetchPartialTreeWithSnapshot).not.toHaveBeenCalled();
  });
  ...
  it('applies incoming action-packet tree nodes without a follow-up subtree fetch', async () => {
    const email = 'reader@example.com';
    const userHandle = 'usrhandle01';
    const ownerHandle = 'owner000001';
    const shareHandle = 'cIVQ2bjB';
    ...
    const filePlaintext = Buffer.from('packet-direct-apply-data', 'utf8');
    const cryptoOps = createCryptoOperations();
    const blockHash = await cryptoOps.computeHash(filePlaintext);
    const fileName = `${blockHash}.bin`;
    const fileCiphertext = encryptFileContent(filePlaintext, fileNodeKey);
    const secretStore = createMemorySecretStore();

    await secretStore.set('provider-account:mega:acct-mega-direct-packet-apply', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(masterKey),
    ...
    const fetchImpl = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      if (typeof init?.body === 'string') {
        ...
        switch (payload.a) {
          ...
          case 'f':
            subtreeFetchCount += 1;
            throw new Error('Unexpected subtree fetch during direct action-packet apply');
          default:
            throw new Error(`Unexpected MEGA API payload: ${JSON.stringify(payload)}`);
        }
      }
      if (String(_input) === 'https://download.test/file') {
        return new Response(new Uint8Array(fileCiphertext), { status: 200 });
      }
      throw new Error(`Unexpected request URL: ${String(_input)}`);
    }) as typeof fetch;
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), 'nearbytes-mega-direct-packet-apply-'));
    tempDirs.push(localPath);
    const account: ProviderAccount = {
      id: 'acct-mega-direct-packet-apply',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-direct-packet-apply',
      provider: 'mega',
      accountId: account.id,
      label: 'Team Space',
      role: 'recipient',
      localPath,
      sourceId: 'src-mega-direct-packet-apply',
      syncMode: 'mirror',
      remoteDescriptor: {
        rootHandle: shareHandle,
        shareHandle,
    ...
    await expect(fs.readFile(path.join(localPath, 'blocks', fileName))).resolves.toEqual(filePlaintext);
  ...
  it('refetches an unknown recipient parent folder so a pushed file handle can resolve immediately', async () => {
    ...
    expect(applied).toMatchObject({ applied: true, contentApplied: true });
    ...
    expect(subtreeFetchCount).toBeGreaterThanOrEqual(2);
    expect(downloadRequestCount).toBe(1);
  ...
  it('ignores deleted handles when a mixed delete plus tree packet can be applied immediately', async () => {
    ...
    const ownerHandle = 'owner000001';
    const shareHandle = 'cIVQ2bjB';
    const deletedHandle = 'deleted001';
    const blocksHandle = 'blocks0001';
    const fileHandle = 'file000011';
    const shareKey = Buffer.from('4f1e2d3c5b6a79888796a5b4c3d2e1f0', 'hex');
    const blocksNodeKey = Buffer.from('51223344556677889900aabbccddeeff', 'hex');
    const fileNodeKey = Buffer.from('40112233445566778899aabbccddeeff202132435465768798a9babbdcddf0f1', 'hex');
    const filePlaintext = Buffer.from('mixed-delete-tree-apply', 'utf8');
    const cryptoOps = createCryptoOperations();
    const blockHash = await cryptoOps.computeHash(filePlaintext);
    const blockFileName = `${blockHash}.bin`;
    const fileCiphertext = encryptFileContent(filePlaintext, fileNodeKey);
    ...
    await secretStore.set('provider-account:mega:acct-mega-mixed-delete-tree', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(Buffer.from('00112233445566778899aabbccddeeff', 'hex')),
    ...
    const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (typeof init?.body === 'string' && url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
          ...
          case 'f':
            subtreeFetchCount += 1;
            throw new Error('Unexpected subtree fetch during mixed immediate apply');
          default:
            throw new Error(`Unexpected MEGA API payload: ${JSON.stringify(payload)}`);
        }
      }
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), 'nearbytes-mega-mixed-delete-tree-'));
    tempDirs.push(localPath);

    const account: ProviderAccount = {
      id: 'acct-mega-mixed-delete-tree',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-mixed-delete-tree',
      provider: 'mega',
      accountId: account.id,
      label: 'Team Space',
      role: 'recipient',
      localPath,
      sourceId: 'src-mega-mixed-delete-tree',
      syncMode: 'mirror',
      remoteDescriptor: {
        rootHandle: shareHandle,
        shareHandle,
  ...
  it('applies a recipient update when the fetched mirror root handle differs from the incoming share descriptor handle', async () => {
    const email = 'reader@example.com';
    const userHandle = 'usrhandle01';
    const ownerHandle = 'owner000001';
    const recipientShareHandle = 'bp5zmCTB';
    const ownerRootHandle = 'P9xVXZrb';
    const blocksHandle = 'bhwkBLYa';
    const fileHandle = 'z1IFiDAZ';
    const shareKey = Buffer.from('4f1e2d3c5b6a79888796a5b4c3d2e1f0', 'hex');
    const rootNodeKey = Buffer.from('102132435465768798a9babbdcddf0f1', 'hex');
    const blocksNodeKey = Buffer.from('11223344556677889900aabbccddeeff', 'hex');
    const fileNodeKey = Buffer.from('00112233445566778899aabbccddeeff102132435465768798a9babbdcddf0f1', 'hex');
    const fileBytes = Buffer.from('recipient-root-handle-mismatch', 'utf8');
    const cryptoOps = createCryptoOperations();
    const blockHash = await cryptoOps.computeHash(fileBytes);
    const fileName = `${blockHash}.bin`;
    const fileCiphertext = encryptFileContent(fileBytes, fileNodeKey);
    const secretStore = createMemorySecretStore();

    await secretStore.set('provider-account:mega:acct-mega-root-handle-mismatch', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(Buffer.from('00112233445566778899aabbccddeeff', 'hex')),
      ...
      if (typeof init?.body === 'string' && url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
          ...
          case 'f':
            subtreeFetchCount += 1;
            if (
              payload.n !== fileHandle &&
              payload.c !== fileHandle &&
              payload.n !== ownerRootHandle &&
              payload.c !== ownerRootHandle
            ) {
              return new Response(
                JSON.stringify([{
                  f: [
                    ...
                    },
                    {
                      ...
                      s: fileBytes.length,
                      a: encryptAttributes(fileName, fileNodeKey),
                      k: encryptNodeKey(fileNodeKey, shareKey, ownerRootHandle),
                      su: ownerHandle,
                    },
                  ],
                  u: [{ u: ownerHandle, m: 'owner@example.com' }],
            ...
            if (payload.n === fileHandle || payload.c === fileHandle) {
              return new Response(
                JSON.stringify([{
                  f: [
                    {
            ...
            if (payload.n === ownerRootHandle || payload.c === ownerRootHandle) {
              return new Response(
                JSON.stringify([{
                  f: [
                    {
            ...
            throw new Error(`Unexpected subtree fetch during root-handle mismatch apply: ${JSON.stringify(payload)}`);
          case 'g':
            downloadRequestCount += 1;
            return new Response(JSON.stringify([{ g: 'https://download.test/file' }]), {
              status: 200,
    ...
    (adapter as unknown as {
      accountShareKeyCache: Map<string, ReadonlyMap<string, Buffer>>;
    }).accountShareKeyCache.set(userHandle, new Map([[ownerRootHandle, shareKey], [recipientShareHandle, shareKey]]));

    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), 'nearbytes-mega-root-handle-mismatch-'));
    tempDirs.push(localPath);

    const account: ProviderAccount = {
      id: 'acct-mega-root-handle-mismatch',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-root-handle-mismatch',
      provider: 'mega',
      accountId: account.id,
      label: 'nearbytes',
      role: 'recipient',
      localPath,
      sourceId: 'src-mega-root-handle-mismatch',
      syncMode: 'mirror',
      remoteDescriptor: {
        rootHandle: recipientShareHandle,
        shareHandle: recipientShareHandle,
        ownerEmail: 'owner@example.com',
        shareName: 'nearbytes',
    ...
    const applied = await (adapter as any).applyRecipientHandleUpdate(
      share,
      await (adapter as any).getAccountSession(account),
      recipientShareHandle,
      fileHandle,
      {},
    ...
    expect(applied).toMatchObject({ applied: true, contentApplied: true });
    await expect(fs.readFile(path.join(localPath, 'blocks', fileName))).resolves.toEqual(fileBytes);
    expect(subtreeFetchCount).toBeGreaterThanOrEqual(2);
    expect(downloadRequestCount).toBe(1);
  ...
  it('falls back from a transient recipient partial fetch without waiting for MEGA lock retries', async () => {
    vi.useFakeTimers();
    try {
      const email = 'reader@example.com';
      const userHandle = 'usrhandle01';
      ...
      const shareHandle = 'cIVQ2bjB';
      const blocksHandle = 'blocks0001';
      const fileHandle = 'file000099';
      const masterKey = Buffer.from('00112233445566778899aabbccddeeff', 'hex');
      const shareKey = Buffer.from('4f1e2d3c5b6a79888796a5b4c3d2e1f0', 'hex');
      const rootNodeKey = Buffer.from('102132435465768798a9babbdcddf0f1', 'hex');
      const blocksNodeKey = Buffer.from('11223344556677889900aabbccddeeff', 'hex');
      const fileNodeKey = Buffer.from('00112233445566778899aabbccddeeff102132435465768798a9babbdcddf0f1', 'hex');
      const fileBytes = Buffer.from('fast-partial-fallback', 'utf8');
      const cryptoOps = createCryptoOperations();
      const blockHash = await cryptoOps.computeHash(fileBytes);
      const fileName = `${blockHash}.bin`;
      const fileCiphertext = encryptFileContent(fileBytes, fileNodeKey);
      ...
      await secretStore.set('provider-account:mega:acct-mega-fast-partial-fallback', {
        email,
        password: 'secret',
        sid: 'helper-session',
        masterKey: encodeMegaBase64Url(masterKey),
      ...
      const adapter = new MegaTransportAdapter(runtime, { fetchImpl });
      const localPath = await fs.mkdtemp(path.join(os.tmpdir(), 'nearbytes-mega-fast-partial-fallback-'));
      tempDirs.push(localPath);
      const account: ProviderAccount = {
        id: 'acct-mega-fast-partial-fallback',
        provider: 'mega',
        label: 'MEGA',
        email,
        state: 'connected',
      ...
      const share: ManagedShare = {
        id: 'share-mega-fast-partial-fallback',
        provider: 'mega',
        accountId: account.id,
        label: 'nearbytes',
        role: 'recipient',
        localPath,
        sourceId: 'src-mega-fast-partial-fallback',
        syncMode: 'mirror',
        remoteDescriptor: {
          rootHandle: shareHandle,
          shareHandle,
      ...
      const appliedPromise = (adapter as any)
      ...
      await expect(appliedPromise).resolves.toMatchObject({ applied: true, contentApplied: true });
      await expect(fs.readFile(path.join(localPath, 'blocks', fileName))).resolves.toEqual(fileBytes);
      expect(partialFetchCount).toBe(1);
      expect(fullFetchCount).toBeGreaterThanOrEqual(1);
      expect(downloadRequestCount).toBe(1);
  ...
  it('skips a stale recipient handle after one missing-root subtree fetch', async () => {
    const email = 'reader@example.com';
    const userHandle = 'usrhandle01';
    const ownerHandle = 'owner000001';
    const shareHandle = 'cIVQ2bjB';
    const staleHandle = 'stale00002';
    const blocksHandle = 'blocks0002';
    const shareKey = Buffer.from('4f1e2d3c5b6a79888796a5b4c3d2e1f0', 'hex');
    const rootNodeKey = Buffer.from('102132435465768798a9babbdcddf0f1', 'hex');
    const blocksNodeKey = Buffer.from('51223344556677889900aabbccddeeff', 'hex');
    ...
    await secretStore.set('provider-account:mega:acct-mega-missing-root-stale-handle', {
      email,
      password: 'secret',
      sid: 'helper-session',
      masterKey: encodeMegaBase64Url(Buffer.from('00112233445566778899aabbccddeeff', 'hex')),
      ...
      if (typeof init?.body === 'string' && url.startsWith('https://g.api.mega.co.nz/cs')) {
        ...
        switch (payload.a) {
          ...
          case 'f':
            ...
            throw new Error(`Unexpected subtree fetch during missing-root immediate apply: ${JSON.stringify(payload)}`);
          default:
            throw new Error(`Unexpected MEGA API payload: ${JSON.stringify(payload)}`);
        }
      }
    ...
    const localPath = await fs.mkdtemp(path.join(os.tmpdir(), 'nearbytes-mega-missing-root-stale-handle-'));
    tempDirs.push(localPath);

    const account: ProviderAccount = {
      id: 'acct-mega-missing-root-stale-handle',
      provider: 'mega',
      label: 'MEGA',
      email,
      state: 'connected',
    ...
    const share: ManagedShare = {
      id: 'share-mega-missing-root-stale-handle',
      provider: 'mega',
      accountId: account.id,
      label: 'Team Space',
      role: 'recipient',
      localPath,
      sourceId: 'src-mega-missing-root-stale-handle',
      syncMode: 'mirror',
      remoteDescriptor: {
        rootHandle: shareHandle,
        shareHandle,
    ...
    const applied = await (adapter as any).applyRecipientHandleUpdate(
      ...
      new Map<string, string>(),
      {
        source: 'sc',
        rootHandle: shareHandle,
        triggerHandle: staleHandle,
        packetReceivedAt: Date.now(),
        scsn: 'cursor-1',
      }
    );

    expect(applied).toMatchObject({ applied: false, contentApplied: false });
    expect(subtreeFetchCount).toBe(1);
